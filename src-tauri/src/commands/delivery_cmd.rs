...
#[tauri::command]
pub fn generate_delivery_note(state: State<'_, DbState>, daily_order_id: String, kitchen_id: String, delivery_date: String) -> Result<DeliveryNote, String> {
    let mut conn = state.0.lock().map_err(|e| e.to_string())?;
    let tx = conn.transaction().map_err(|e| e.to_string())?;

    let kitchen_code: String = tx.query_row("SELECT code FROM kitchens WHERE id=?1", [&kitchen_id], |r| r.get(0)).map_err(|e| format!("Kitchen not found: {}", e))?;
    ...
    let fetched_items: Vec<(String,String,f64,String)> = {
        let mut stmt = tx.prepare("SELECT id,product_name,quantity,unit FROM order_items WHERE daily_order_id=?1 AND kitchen_id=?2 AND category IN ('internal','operational')").map_err(|e| e.to_string())?;
        let rows = stmt.query_map(rusqlite::params![daily_order_id, kitchen_id], |r| Ok((r.get::<_,String>(0)?,r.get::<_,String>(1)?,r.get::<_,f64>(2)?,r.get::<_,String>(3)?))).map_err(|e| e.to_string())?;
    ...
    Ok(DeliveryNote { id, daily_order_id, kitchen_id, kitchen_name: Some(kitchen_name), delivery_number, delivery_date, status: "draft".into(), notes: None, item_count: ic, created_at: None })
}

#[tauri::command]
pub fn get_delivery_notes(state: State<'_, DbState>, daily_order_id: Option<String>) -> Result<Vec<DeliveryNote>, String> {
    ...
    let query = if daily_order_id.is_some() { format!("{} WHERE dn.daily_order_id=?1 ORDER BY dn.delivery_date DESC", base) } else { format!("{} ORDER BY dn.delivery_date DESC LIMIT 50", base) };
    let mut stmt = conn.prepare(&query).map_err(|e| e.to_string())?;

    let map_row = |row: &rusqlite::Row| -> rusqlite::Result<DeliveryNote> {
        Ok(DeliveryNote { id: row.get(0)?, daily_order_id: row.get(1)?, kitchen_id: row.get(2)?, kitchen_name: row.get(3)?, delivery_number: row.get(4)?, delivery_date: row.get(5)?, status: row.get(6)?, notes: row.get(7)?, created_at: row.get(8)?, item_count: row.get(9)? })
    };
    ...
    for r in rows { if let Ok(n) = r { notes.push(n); } }
    Ok(notes)
}

#[tauri::command]
pub fn get_delivery_note_detail(state: State<'_, DbState>, note_id: String) -> Result<DeliveryNoteDetail, String> {
    let conn = state.0.lock().map_err(|e| e.to_string())?;
    ...
    let rows = stmt.query_map([&note_id], |r| Ok(DeliveryNoteItem { id: r.get(0)?, product_name: r.get(1)?, quantity: r.get(2)?, unit: r.get(3)? })).map_err(|e| e.to_string())?;
