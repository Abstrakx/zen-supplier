...
export const StoreModal: React.FC<Props> = ({
  ...
  const handleSave = async () => {
    if (!name.trim()) {
      Swal.fire("Perhatian", "Nama toko harus diisi", "warning");
      return;
    }
    ...
    try {
      ...
      Swal.fire({
        icon: "success",
        title: "Berhasil",
        text: `Toko ${initialData ? "diperbarui" : "ditambahkan"}`,
        timer: 1500,
      ...
      <div className="bg-white w-full max-w-lg rounded-2xl overflow-hidden shadow-2xl animate-in zoom-in-95 duration-300">
        <div className="px-6 py-4 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
          <h3 className="font-black text-slate-800 flex items-center gap-2 uppercase tracking-tight text-sm">
            ...
            {initialData ? "Edit Data Toko" : "Tambah Toko Baru"}
          </h3>
          <button
            onClick={onClose}
        ...
        <div className="p-6 space-y-4">
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <input
                className="w-full p-3 bg-slate-50 border border-slate-200 rounded-xl outline-none focus:border-violet-500 transition-all font-bold text-slate-700"
                value={name}
                onChange={(e) => setName(e.target.value)}
                placeholder="Toko Bude Pur"
              />
            </div>
            <div>
              <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1 block">
        ...
        <div className="px-6 py-4 border-t border-slate-100 flex justify-end gap-2 bg-slate-50/50">
          ...
          <button
            onClick={handleSave}
            disabled={loading}
            className="bg-violet-600 text-white px-6 py-2 rounded-xl font-bold shadow-lg shadow-violet-500/20 hover:bg-violet-700 transition-all flex items-center gap-2 disabled:opacity-50"
          >
            {loading ? (
