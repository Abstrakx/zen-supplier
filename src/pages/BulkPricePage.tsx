...
export const BulkPricePage: React.FC = () => {
  ...
  const productList = useMemo(() => {
    return products.filter((p) => {
      ...
      const matchesType =
        selectedCategory === "all" || p.item_type === selectedCategory;
  ...
  const getProductDirtyCount = (productId: string) => {
    return rows.filter(
      (r) => r.product_id === productId && (r.isDirtyBuy || r.isDirtySell),
    ).length;
  };
        ...
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center py-32 text-center bg-white rounded-[40px] border-2 border-dashed border-slate-100 shadow-sm">
            ...
            <p className="text-sm text-slate-400 mt-2 font-medium max-w-xs leading-relaxed">
              Coba gunakan kata kunci lain atau ubah filter kategori untuk
              menemukan produk.
            </p>
          </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {productList.map((p) => {
              ...
              const baseRow = pRows.find(
                (r) => p.units.find((u) => u.id === r.unit_id)?.is_base_unit,
              );
              ...
              <div className="flex items-center gap-4">
                ...
                <div>
                  <span className="text-[10px] font-black text-slate-400 uppercase tracking-[0.3em] mb-1 block">
                    {editingProduct.item_type === "dapur"
                      ? "Bahan Dapur"
                      : "Operasional"}
                  </span>
            ...
            <div className="flex-1 overflow-y-auto p-10">
              <div className="mb-8 p-4 bg-blue-50 border border-blue-100 rounded-2xl flex items-start gap-3">
                ...
                <p className="text-xs text-blue-700 font-medium leading-relaxed">
                  Gunakan tabel di bawah untuk mengatur harga setiap satuan.
                  Unit dengan konversi lebih besar dari 1 adalah satuan turunan
                  dari <b>{editingProduct.base_unit}</b>.
                </p>
              </div>
              ...
              <table className="w-full text-left">
                <thead>
                  <tr className="border-b border-slate-100">
                    <th className="pb-4 px-4 text-[10px] font-black text-slate-400 uppercase tracking-widest">
                      Satuan & Konversi
                    </th>
                    <th className="pb-4 px-4 text-[10px] font-black text-slate-400 uppercase tracking-widest">
                      Status
                    </th>
                    <th className="pb-4 px-4 text-[10px] font-black text-slate-400 uppercase tracking-widest text-right">
                      Harga Beli (Modal)
                    </th>
                    <th className="pb-4 px-4 text-[10px] font-black text-slate-400 uppercase tracking-widest text-right">
                      Harga Jual (Dapur)
                ...
                <tbody className="divide-y divide-slate-50">
                  {editingProduct.units
                    .sort((a, _b) => (a.is_base_unit ? -1 : 1))
                    .map((unit) => {
                      const row = rows.find((r) => r.unit_id === unit.id);
                      if (!row) return null;

                      const baseUnit = editingProduct.units.find(
                        (u) => u.is_base_unit,
                      );
                      const baseRow = rows.find(
                        (r) => r.unit_id === baseUnit?.id,
                      );

                      // Inheritance check
                      const inheritedBuy =
                        (baseRow?.new_buy || 0) * unit.conversion_to_base;
                      const inheritedSell =
                        (baseRow?.new_sell || 0) * unit.conversion_to_base;
                      const isAutoBuy =
                        !unit.is_base_unit && row.new_buy === inheritedBuy;
                      const isAutoSell =
                        !unit.is_base_unit && row.new_sell === inheritedSell;

                      return (
                        <tr
                          key={unit.id}
                          className={`group ${unit.is_base_unit ? "bg-orange-50/30" : ""}`}
                        >
                          <td className="py-6 px-4">
                            <div className="flex flex-col">
                              <div className="flex items-center gap-2">
                                ...
                                {unit.is_base_unit && (
                                  <span className="px-2 py-0.5 bg-orange-100 text-orange-600 text-[8px] font-black rounded uppercase">
                                    Base Unit
                                  </span>
                                )}
                              </div>
                              {!unit.is_base_unit && (
                                <span className="text-[10px] font-bold text-slate-400 mt-1 uppercase tracking-wider">
                                  1 {unit.unit_name} = {unit.conversion_to_base}{" "}
                                  {editingProduct.base_unit}
                                </span>
                              )}
                            </div>
                          </td>
                          ...
                          <td className="py-6 px-4">
                            <div className="flex flex-col items-end gap-2">
                              <CurrencyInput
                                ...
                                value={row.new_buy || 0}
                                onChange={(val) =>
                                  handlePriceChange(unit.id, "buy", val)
                                }
                                prefix="Rp"
                              />
                              <div className="flex items-center gap-1">
                                <span className="text-[9px] font-bold text-slate-400 uppercase tabular-nums">
                                  Current: Rp{" "}
                                  {(row.current_buy || 0).toLocaleString(
                                    "id-ID",
                                  )}
                                </span>
                          ...
                          <td className="py-6 px-4">
                            <div className="flex flex-col items-end gap-2">
                              <CurrencyInput
                                ...
                                value={row.new_sell || 0}
                                onChange={(val) =>
                                  handlePriceChange(unit.id, "sell", val)
                                }
                                prefix="Rp"
                              />
                              <div className="flex items-center gap-1">
                                <span className="text-[9px] font-bold text-slate-400 uppercase tabular-nums">
                                  Current: Rp{" "}
                                  {(row.current_sell || 0).toLocaleString(
                                    "id-ID",
                                  )}
                                </span>
